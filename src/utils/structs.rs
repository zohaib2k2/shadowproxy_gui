    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let hashmap = match json_str_to_hashmap(self.headers.as_str()) {
            ...
            Err(e) => {
                ...
                out_put.push_str(&format!(
                    "Failed to parse headers another L for me: {}\n",
                    e
                ));
...
impl FromStr for HttpMethod {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => Err(format!(
                "Unsupported HTTP method zzz worng shit: {}",
                s.to_string()
            )),
        }
    }
...
impl FromStr for RequestDataProper {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let full_url = format!(
            "{}://{}/{}",
            scheme,
            host.trim().trim_end_matches("/").trim_start_matches("/"),
            url.trim().trim_end_matches("/").trim_start_matches("/")
        );
